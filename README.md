<h1 align="center">Hi 👋, I'm Nikhil Chandra</h1>
<h3 align="center">
Electronics Engineer | DevOps-Oriented Full Stack Developer | ISRO Research Contributor
</h3>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com/?lines=Electronics+Engineer+🚀;DevOps+%26+Cloud+Enthusiast+☁️;Full+Stack+Web+Developer+💻;ISRO+Research+Project+Contributor+🛰️;Problem+Solver+%26+Lifelong+Learner+📚&center=true&width=700&height=50" />
</p>

<p align="center">
  <img alt="coding" width="600" height="350" src="https://user-images.githubusercontent.com/55389276/140866485-8fb1c876-9a8f-4d6a-98dc-08c4981eaf70.gif" />
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=nik03072005&label=Profile%20Views&color=0e75b6&style=flat" />
</p>

---

## 👨‍🚀 Professional Summary (Recruiter Optimized)

Final-year **Electronics Engineering** student with strong expertise in  
**DevOps-aware Full Stack Development**, **Cloud-native systems**, and **Research-grade Data Visualization**.

Currently contributing to an **ISRO RESPOND–funded project**, building scalable GUI systems for **MIL-STD-1553B spacecraft onboard data analysis**.  
Hands-on experience in **React, Node.js, Docker, CI/CD concepts, PostgreSQL, and system-level problem solving**.

Actively seeking **Software Engineer / DevOps / Research Intern / Graduate Engineer** roles.

---

## 🛰️ ISRO Research Experience

### 🚀 ISRO RESPOND Project — *Frontend & System Design Engineer*
**GUI-Based MIL-STD-1553B Spacecraft Onboard Data Analyzer**

- Designed interactive dashboards for **spacecraft bus communication analysis**
- Visualized **jitter, periodicity, deviation, and protocol violations**
- Built scalable frontend architecture for research-grade data pipelines
- Worked with real spacecraft communication trace data
- Followed **defensive coding, modular design, and performance optimization**

**Tech:** React.js, D3.js, Chart.js, Node.js, PostgreSQL  
**Domain:** Avionics | Spacecraft Data Bus | Research Systems

---

## 🧠 Internships & Training

### 🔹 ISRO Project Internship (Ongoing)
- Government-funded research project (RESPOND)
- GUI + visualization for avionics communication protocols

### 🔹 VHDL / Digital Design Internship
- Sequential & combinational circuit design
- FSMs, ALUs, sequence detectors, stopwatch design
- RTL-level thinking & hardware-software co-design

---

## 📜 Certifications

- ✅ Full Stack Web Development (MERN Stack)
- ✅ UI/UX Design with Figma
- ✅ Linux & Git Fundamentals
- ✅ Data Structures & Algorithms (Ongoing)
- ✅ Cloud & DevOps Fundamentals (Docker, CI/CD – Ongoing)

---

## 🧰 Tech Stack (DevOps + Full Stack)

### ⚙️ Languages
`JavaScript` `Python` `C` `VHDL` `SQL`

### 🌐 Frontend
`React.js` `Tailwind CSS` `GSAP` `D3.js` `Chart.js`

### 🧩 Backend
`Node.js` `Express.js` `REST APIs` `JWT Auth`

### 🗄️ Databases
`PostgreSQL` `MongoDB`

### ☁️ DevOps & Tools
`Docker` `Git & GitHub` `Linux` `CI/CD Concepts` `Cloud Fundamentals`

### 🎨 UI/UX
`Figma` `Design Systems` `Accessibility`

---

## 📊 Animated Skill Bars

**Frontend Development**
![Frontend](https://progress-bar.dev/90/?title=Frontend&color=61dafb)

**Backend Development**
![Backend](https://progress-bar.dev/80/?title=Backend&color=brightgreen)

**DevOps & Cloud**
![DevOps](https://progress-bar.dev/70/?title=DevOps+%26+Cloud&color=blue)

**Data Visualization & Research**
![Research](https://progress-bar.dev/85/?title=Research+%26+Visualization&color=orange)

---

## 🚀 Featured Projects

### 📺 Video Hosting Platform
- Scalable video upload & streaming
- Secure authentication & role-based access
- Cloud media handling with performance optimization

**Tech:** React, Node.js, Express, MongoDB, Cloudinary, JWT

---

### 🌐 Perfume E-Commerce Platform
- Authentication, cart, admin dashboard
- Responsive & optimized UI
- Production-ready backend APIs

**Tech:** React, Tailwind, Node.js, MongoDB

---

### 🍎 Apple iPhone 15 Website Clone
- Pixel-perfect UI
- Smooth scrolling & animation-heavy experience
- Apple-style design system

**Tech:** React, Tailwind CSS, GSAP

---

## 🏆 GitHub Achievements

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=nik03072005&theme=radical&no-frame=true&row=1&column=7" />
</p>

---

## 📈 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=nik03072005&show_icons=true&theme=radical" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=nik03072005&theme=radical" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=nik03072005&layout=compact&theme=radical" />
</p>

---

## 🌐 Connect With Me

<p align="left">
  <a href="https://linkedin.com/in/nikhil-chandra-783581257" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-Professional-blue?style=for-the-badge&logo=linkedin" />
  </a>
  <a href="mailto:nc03072005@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail" />
  </a>
</p>

---

⭐ *Open to DevOps, Software Engineering, Research & Graduate Engineer roles*  
⭐ *Let’s build scalable systems that matter*
